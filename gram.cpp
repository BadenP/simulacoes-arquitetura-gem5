...
#include <random> 
#include "math.h"
#ifndef NUMBER_VECTORS
#define NUMBER_VECTORS 100
#endif
/**
 ...
 */
int main() {
    int r = NUMBER_VECTORS;
    int c = NUMBER_VECTORS;
    test();  // perform self tests

    std::array<std::array<double, 10>, 20> A{};
    std::array<std::array<double, 10>, 20> B = {{0}};
    ...
    // Generate random values for vectors in array A
    for (int i = 0; i < r; ++i) {
        std::cout << "Enter vector " << i + 1 << '\n';
        for (int j = 0; j < c; ++j) {
            A[i][j] = dist(gen); // Generate random value within [-50, 50]
        }
        std::cout << '\n';
    }
